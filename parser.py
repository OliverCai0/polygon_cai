...
def parse_file( fname, edges, polygons, transform, screen, color ):
    ...
    while c < len(lines):
        ...
        if line == 'sphere':
            ...
            f = [float(x) for x in args[:]]
            add_box(polygons,f[0],f[1],f[2],f[3],f[4],f[5])
            #print 'BOX\t' + str(args)
            # add_box(edges,
            #         float(args[0]), float(args[1]), float(args[2]),
            ...
            ident(transform)
             ...
             #for triangle in polygons:
                #print_matrix(transform)
                #matrix_mult(transform,triangle)
             #matrix_mult(transform,polygons[11])
             for i in range(len(polygons)):
                 temp = copy.deepcopy(polygons[i])
